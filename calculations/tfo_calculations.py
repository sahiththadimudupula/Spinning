...
def get_tfo_divisor(row: pd.Series) -> float:
    explicit_divisor = safe_float(row.get("TFO Divisor"), 0.0)
    if explicit_divisor > 0:
        return explicit_divisor
    if normalize_key_text(row.get("Count")) == normalize_key_text(SPECIAL_TFO_COUNT):
        return SPECIAL_TFO_DIVISOR
    return DEFAULT_TFO_DIVISOR



def get_machine_divisor(row: pd.Series) -> float:
    explicit_divisor = safe_float(row.get("Machine Divisor"), 0.0)
    return explicit_divisor if explicit_divisor > 0 else DEFAULT_MACHINE_DIVISOR



def build_formula_texts(excel_row: int, divisor: float, machine_divisor: float) -> dict[str, str]:
    divisor_text = int(divisor) if divisor == int(divisor) else divisor
    machine_divisor_text = int(machine_divisor) if machine_divisor == int(machine_divisor) else machine_divisor

    return {
        "Production per Drum/day Formula": f"=((D{excel_row}*60*8*G{excel_row}*2)/(E{excel_row}*36*840*C{excel_row}*2.202))*3",
        "Production Required / Month Kgs Formula": f"=(Production Required / day Kgs) * 30",
        "No. of Drums Required Formula": f"=(Production Required / day Kgs)/(Production per Drum/day)",
        "TFO Reqd./ Shift Formula": f"=(No. of Drums Required)/{divisor_text}",
        "kgs/drum/day Formula": f"=((J{excel_row}*K{excel_row}*8*60*1.09)/(C{excel_row}*840*2.202))*3",
        "No. of Drums (A/W) Reqd. Formula": f"=(Production Required / day Kgs)/(kgs/drum/day)",
        "Assembly Winding Reqd./ Shift Formula": f"=(No. of Drums (A/W) Reqd.)/{machine_divisor_text}",
    }
...
def calculate_tfo_production_dataframe(tfo_input_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for row_number, row in tfo_input_df.reset_index(drop=True).iterrows():
        ...
        divisor = get_tfo_divisor(row)
        machine_divisor = get_machine_divisor(row)
        ...
        assembly_winding_required_shift = no_of_drums / machine_divisor if machine_divisor > 0 else 0.0
        ...
        output_row = {
            ...
            "TFO Divisor": round(divisor, 2),
            "Machine Divisor": round(machine_divisor, 2),
            "__excel_row": excel_row,
        }
        output_row.update(build_formula_texts(excel_row=excel_row, divisor=divisor, machine_divisor=machine_divisor))
...
def calculate_tfo_manpower_dataframe(production_df: pd.DataFrame) -> pd.DataFrame:
    special_mask = production_df["Count"].apply(normalize_key_text) == normalize_key_text(SPECIAL_TFO_COUNT)
    special_df = production_df.loc[special_mask].copy()
    non_special_df = production_df.loc[~special_mask].copy()
    ...
    base_rows = [
        (
            ...
            "Assembly winding",
            "operator",
            "ROUNDUP((((Sum Non-Special No. of Drums (A/W) Reqd./36) + (Sum Special No. of Drums (A/W) Reqd./36)) * 3),0)",
            assembly_winding,
        ),
        (2, "Jumbo Assembly Winding", "operator", "ROUND(ROUNDUP(Sum Special No. of Drums (A/W) Reqd.,0)/16,0)*2", jumbo_assembly_winding),
        (3, "TFO", "TFO Operator", "ROUND((Sum Non-Special TFO Reqd./ Shift)/6,0)*3", tfo_operator),
        (4, "TFO", "TFO Operator (Doffer)", "ROUNDUP((Sum Non-Special TFO Reqd./ Shift)/4,0)*3", tfo_operator_doffer),
